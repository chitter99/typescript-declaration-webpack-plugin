...
export default class TypescriptDeclarationPlugin extends optimize.ModuleConcatenationPlugin {
    ...
    private regexImportDirective =
        /(\/{3} ?<reference types=")([A-Za-z0-9-]+)(" ?\/>)/;
    private regexImportModule =
        /import ({.+}|\*) as ([A-Za-z0-1-]+) from ('|")([A-Za-z0-1-]+)('|");/;
    ...
    apply(compiler: Compiler) {
        compiler.hooks.emit.tapAsync(this.name, (compilation, callback) => {
            ...
            if (declarationFiles.length > 0) {
                const mergedDeclarationFile =
                    this.mergeDeclarations(declarationFiles);
                ...
                this.log(
                    'Make sure declarations are activated in tsconfig.json!'
                );
            }
